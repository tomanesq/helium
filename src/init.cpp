...
#include "zsqr/accumulatorcheckpoints.h"
#include "zsqrchain.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZsqrThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-sqrstake=<n>", strprintf(_("Enable or disable staking functionality for SQR inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsqrstake=<n>", strprintf(_("Enable or disable staking functionality for zSQR inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzsqr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSQR minting (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zsqrbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSQR backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-2020 The Helium Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The sQuorum Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZsqrThread(threadGroup);
    }
